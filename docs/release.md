...
2. Runs the Python tests and the shared Gradle `verify` gate (quality checks,
   patch tests, both extension test suites, and bundle verification); the single
   `.mpp` must be named `patches-<version>.mpp`.
3. Creates the GitHub release (marked latest) with the notes and the `.mpp`.
   A retry reuses the existing release and uploads a missing asset instead of
   failing. `attest-build-provenance` attests the bundle.
