...
| A re-signed Zalo APK can fail before Java startup because `libnative_utils.so` validates the original certificate; patching one observed `apk_tampered` exit branch is not sufficient. | On Galaxy SM-S936B / Android 16, an all-patches-disabled Morphe build still exited immediately after loading `libnative_utils.so`; the stock APK remained alive. The safe response is to trace every validation/configuration path and preserve initialization, not suppress a shared exit helper. The Java bridge is `xo1/a1` → `InitializeConfig.a(...)`; it first calls `xo1/f1.D(...NATIVE_UTILS)` and only invokes native initialization when `xo1/f1.B(...)` reports the library loaded. This makes the loader/registry result a separate failure edge from native configuration initialization. |
| A native startup mutation must be device-validated before it is retained or enabled. | A version/ABI/hash-guarded arm64 candidate that redirected one certificate-failure edge to cleanup still exited immediately after `libnative_utils.so` loaded. It was removed and must not be treated as a working bypass. |
| Static signature-path finding for Zalo 26.08.01. | `libnative_utils.so` contains JNI lookup/use evidence for `android/content/pm/PackageManager`, `getPackageInfo`, `(Ljava/lang/String;I)Landroid/content/pm/PackageInfo;`, `android/content/pm/PackageInfo`, `signatures`, `[Landroid/content/pm/Signature;`, `Signature.toByteArray`, and `getPackageName`. The generic JNI helper at `0x2d1e8` calls `FindClass` (JNI table offset `0x30`), `GetMethodID` (`0x108`), and invokes the resolved method; the native initialization path supplies the PackageManager class and `getPackageInfo` method strings. This is direct evidence that certificate data is obtained through Android's PackageManager API. The library also imports `fopen`/`fread` and uses `openat`/`read`/`stat` for other checks/configuration, and contains `base.apk`/`.apk` strings, but static evidence found no `APK Sig Block 42` magic or a proven raw-signing-block parser. `xo1/a1` calls `f1.D(...NATIVE_UTILS)`, checks `f1.B(...)`, and only then calls `InitializeConfig.a(...)`; no Java PackageManager call feeds the native initializer first. LSPatch-style PackageManager spoofing is therefore viable in principle for this observed certificate path, but not proven sufficient while the native file-I/O path remains unresolved. Runtime Frida hooks should trace `Context.getPackageManager`, `PackageManager.getPackageInfo`, `PackageInfo.signatures`/`signingInfo`, `Signature.toByteArray`, native `InitializeConfig.initialize`, and libc `openat`/`read`/`mmap` with paths; this was not run. |
| Native validation trace follow-up (Zalo 26.08.01, arm64-v8a). | The separate `zalo-patch` checkout contains no `libnative_utils.so` analysis or native patch; its `ZaloArtifactIdentity` is an unrelated Java-side PackageManager/hash utility and targets 26.08.02. In the pinned 26.08.01 library, the `apk_tampered` string is referenced at `InitializeConfig.initialize+0xfc0` (`0x298a4`) and is created through a JNI string call before a native helper dispatches the failure data; this is an error-report path, not proof that the preceding predicate is the only validator. The confirmed PM path is reached through JNI method lookup: at `0x290a4` the native code passes `PackageManager`, `getPackageInfo`, and `(Ljava/lang/String;I)Landroid/content/pm/PackageInfo;`; subsequent JNI calls use `PackageInfo`/`signatures`/`Signature.toByteArray` strings. The apparent `fopen` path (`0x284c0`) is a generic caller-supplied Java-string file reader (`fopen`, seek-to-end, `ftell`, `fread`, `fclose`), not an APK-signing parser by itself. The `InitializeConfig` `openat`/`read`/`stat` sequence checks many filesystem paths and includes root/tooling indicators (`/system/bin/su`, `/data/local/su`, etc.); it is consistent with environment/root/config validation, while the `base.apk` literal has not been proven to reach a signing-block parser. No `APK Sig Block 42` literal or direct `mmap`-based parser was found. Therefore the PM certificate path is mapped, but all predicates leading to the `apk_tampered` report and any use of `base.apk` remain unresolved. Minimal null-outs cannot be proposed safely until dynamic tracing correlates predicates with the failure on hardware; no patch was implemented. Required follow-up is Frida tracing of the JNI calls and `openat`/`fopen`/`read`/`stat`/`mmap`, including resolved paths and return values, plus breakpoints at `0x298a4` and the native entrypoint. |
